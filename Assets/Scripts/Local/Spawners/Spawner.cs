...
public class Spawner : MonoBehaviour
{
    ...
    private void SetLotterie()
    {
        ...
        if (thief)
        {
            models.Add(SpawnerManager.instance.thiefModel);
            rangeRandom++;
        }
        if (warrior)
        {
            models.Add(SpawnerManager.instance.warriorModel);
            rangeRandom++;
        }
        if (mage)
        {
            models.Add(SpawnerManager.instance.mageModel);
            rangeRandom++;
        }
    }
    ...
    private void AddEntity()
    {
        ...
        SpawnerManager.instance.AddEntites(go);

        go.GetComponent<Enemy>().SetExtractingPoint(gameObject);
        go.GetComponent<Enemy>().HaveSteal = false;
    }
