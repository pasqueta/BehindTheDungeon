...
Shader "Custom/VertexShaderDisparition" { //Nom du shader dans la liste des shaders
    Properties {
       //Liste des données parametrables par l'utilisateur en code ou dans l'inspector
		_MainTex("Texture 1", 2D) = "white"{}
		_CubeMask("Mask", 2D) = "white"{}
		_Progression ("Progerssion", Range (0,1)) = 0
    }
    SubShader {
        Pass {
            CGPROGRAM
            ...
            {
                //return lerp(tex2D(_MainTex , i.uv.xy), tex2D(_MainTex2, i.uv.xy),tex2D(_CubeMask , i.uv.xy));

            	float4 blend = tex2D(_CubeMask , i.uv.xy);
            	// float4 ground = tex2D(_MainTex , i.uv.xy);
            	// float4 grass = tex2D(_MainTex2 , i.uv.xy);

				// float3 lightToObject = _LightPos - i.position;
				// lightToObject = normalize(lightToObject);
				// float factor = max(0, dot(i.norm, lightToObject));

				//float4 color = (blend.r > _Progression) ? ground : grass;

				clip(blend.r - _Progression);

				return tex2D(_MainTex, i.uv.xy);

				//return (color * _AmbientColor) + factor * color * _LightColor;
            }
            ENDCG
        }
    }
